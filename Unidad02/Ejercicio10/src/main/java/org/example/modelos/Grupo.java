...
@Table(name = "grupos")
@NamedQuery(
        name = "Grupo.findByMinCanciones",
        query = "SELECT g FROM Grupo g WHERE SIZE(g.canciones) > :numCanciones"
)
@NamedQuery(
        name = "Grupo.findByLocalidadAndAnio",
        query = "SELECT g FROM Grupo g WHERE g.localidad = :localidad AND g.annoGrab < :anio"
)
public class Grupo {
    @Id
